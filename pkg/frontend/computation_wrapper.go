...
func (cwft *TxnComputationWrapper) Compile(requestCtx context.Context, fill func(*batch.Batch) error) (interface{}, error) {
	...
	if _, ok := cwft.stmt.(*tree.Execute); ok {
		...
		retComp, plan, stmt, sql, err := replacePlan(requestCtx, cwft.ses.(*Session), cwft, executePlan)
		if err != nil {
			return nil, err
		}
		originSQL = sql
		...
		if retComp == nil {
			...
			// retComp
			retComp.ResetForRun(getStatementStartAt(requestCtx))
...
func replacePlan(requestCtx context.Context, ses *Session, cwft *TxnComputationWrapper, execPlan *plan.Execute) (*compile.Compile, *plan.Plan, tree.Statement, string, error) {
	originSQL := ""
	...
	for _, obj := range preparePlan.GetSchemas() {
		...
		if newObj == nil {
			return nil, nil, nil, originSQL, moerr.NewInternalError(requestCtx, "table '%s' in prepare statement '%s' does not exist anymore", obj.ObjName, stmtName)
		}
		if newObj.Obj != obj.Obj || newTableDef.Version != uint32(obj.Server) {
			return nil, nil, nil, originSQL, moerr.NewInternalError(requestCtx, "table '%s' has been changed, please reset prepare statement '%s'", obj.ObjName, stmtName)
		}
	}
	...
	if prepareStmt.params != nil && prepareStmt.params.Length() > 0 { // use binary protocol
		if prepareStmt.params.Length() != numParams {
			return nil, nil, nil, originSQL, moerr.NewInvalidInput(requestCtx, "Incorrect arguments to EXECUTE")
		}
