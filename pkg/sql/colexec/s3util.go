...
func (w *S3Writer) SortAndSync(ctx context.Context, proc *process.Process) ([]objectio.BlockInfo, objectio.ObjectStats, error) {
	...
	for i := range w.batches {
		err := SortByKey(proc, w.batches[i], w.sortIndex, w.isClusterBy, proc.GetMPool(), []int64{})
		if err != nil {
			return nil, objectio.ObjectStats{}, err
		}
	}
...
func SortByKey(proc *process.Process, bat *batch.Batch, sortIndex int, allow_null bool, m *mpool.MPool, sels []int64) error {
	hasNull := false
