...
func (p *Pipeline) Run(r engine.Reader, topValueMsgTag int32, proc *process.Process) (end bool, err error) {
	...
	if arg, ok := p.instructions[0].Arg.(*table_scan.Argument); ok {
		arg.Reader = r
		arg.TopValueMsgTag = topValueMsgTag
		arg.Attrs = p.attrs
		arg.TableID = p.tableID
	}
...
func (p *Pipeline) ConstRun(bat *batch.Batch, proc *process.Process) (end bool, err error) {
	...
	if arg, ok := p.instructions[0].Arg.(*value_scan.Argument); ok {
		arg.Batchs = pipelineInputBatches
	}
